...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        with self.update_sampling_params(**kwargs):
            ...
            resp_idx = 0
            n = 8         # ← 直接拿 ##### 改
            prefix_a_ids = self.tokenizer.encode("[General_mode]\n", add_special_tokens=False)
            prefix_b_ids = self.tokenizer.encode("[Personalized_mode]\n", add_special_tokens=False)
            for output in outputs:
                for sample_id in range(len(output.outputs)):
                    response_ids = output.outputs[sample_id].token_ids
                    ### 加前缀
                    if not is_validate:
                        style = 0 if (resp_idx % n) < (n // 2) else 1
                        prefix = prefix_a_ids if style == 0 else prefix_b_ids
                        response_ids = (prefix + response_ids)[:self.config.response_length]
                        resp_idx += 1
                    response.append(response_ids)
                    if self.config.calculate_log_probs:
                        curr_log_prob = []
                        original_offset = len(prefix)  
                        for i, logprob_dict in enumerate(output.outputs[sample_id].logprobs):
                            # 只处理原始采样部分
                            if i < original_offset:
                                continue
                            token_id = response_ids[i]
                            if token_id in logprob_dict:
                                curr_log_prob.append(logprob_dict[token_id].logprob)
                            else:
                                # 找不到就用 -inf 或 0 占位，保证长度对齐
                                curr_log_prob.append(-float('inf'))
